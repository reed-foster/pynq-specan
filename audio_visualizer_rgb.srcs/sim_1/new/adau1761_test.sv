...
bit lfsr_locked = 1'b0; // it will take a sample to lock; the first sample after coming out of reset will be zero.
...
always_ff @(posedge clk) begin
  if (!reset & enabled) begin
    ...
    if (adc_sample.valid) begin
      ...
      if ((adc_sample.data !== output_lfsr) && lfsr_locked) begin
